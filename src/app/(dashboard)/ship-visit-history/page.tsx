...
import { FaShip, FaBox, FaUserTie, FaFlag, FaWeightHanging } from 'react-icons/fa';
import { MdHistory, MdLocalShipping } from 'react-icons/md';
...
type ShipVisit = {
    ...
    country: string;
    agent: string;
    flag: string; // national flag emoji or ISO code
    grossTonnage: number; // in tons
    terminal: string;
};
...
export default function ShipVisitTracker() {
    const [selectedVesselId, setSelectedVesselId] = useState('MV001');
  ...
  <div className="max-w-6xl mx-auto bg-white p-8 rounded-xl shadow-lg">
    ...
    <div className="bg-blue-50 p-6 rounded-lg shadow-inner">
      ...
      {visits.length > 0 ? (
            ...
            <tbody>
              {visits.map((v, i) => (
                <tr key={i} className="border-t border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">{v.country} {v.flag}</td>
        ...
        <p className="text-red-600 text-sm mt-2">No visits found for selected vessel/year.</p>
      )}
    </div>
  </div>
</div>
